...
let cx = 0, cy = 0, SCALE = 1; // updated on resize
...
const rings = [
  { data: days,   radius: 80,  font: "18px monospace", ringPadding: 50 },
  { data: months, radius: 150, font: "16px monospace", ringPadding: 20 },
  { data: dates,  radius: 190, font: "16px monospace", ringPadding: 16 },
  { data: hours,  radius: 225, font: "14px monospace", ringPadding: 16 },
  { data: minutes,radius: 260, font: "14px monospace", ringPadding: 16 },
  { data: seconds,radius: 295, font: "12px monospace", ringPadding: 16 },
  { data: years,  radius: 338, font: "14px monospace", ringPadding: 18 },
];
...
function drawGoalDots(ctx, cx, cy, radius) {
  ...
  goalsData.forEach(goal => {
    ...
    const day = goalDate.getDate(); // 1 - 31
    const angle = (day / 31) * 2 * Math.PI - Math.PI / 2; // map to circle (approx)

    const x = cx + radius * Math.cos(angle);
    const y = cy + radius * Math.sin(angle);

    // draw outer glow small
    ctx.beginPath();
    ...
    ctx.fillStyle = "#F4A6B8"; // pastel pink consistent
    ctx.fill();
...
function drawClock() {
  ...
  // Draw goal dots on the date ring (use ring index 2 radius)
  const dateRingRadius = rings[2].radius * SCALE;
  drawGoalDots(ctx, cx, cy, dateRingRadius);
  ...
  if (hoveredDot) {
    ...
    ctx.fillStyle = "#4B2E2E"; // brown
    ctx.textAlign = "left";
    ctx.textBaseline = "bottom";

    // simple background for tooltip
    const padding = 6 * SCALE;
    const metrics = ctx.measureText(text);
    const tw = metrics.width + padding * 2;
    const th = (12 * SCALE) + padding;
    ...
    roundRect(ctx, tx - padding, ty - th, tw, th, 6 * SCALE, true, true);

    ctx.fillStyle = "#4B2E2E";
    ctx.fillText(text, tx, ty - padding / 2);
    ctx.restore();
  }
